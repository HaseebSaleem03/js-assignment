...
function selectBrand() {
  ...
  productSelect.innerHTML = '<option disabled selected value="">Select Product</option>';
  if (watches[brand]) {
      for (const product in watches[brand]) {
          const option = document.createElement('option');
          option.value = product;
          option.textContent = watches[brand][product].name;
...
function searchProduct() {
  ...
  if (!brand) {
      ...
      productMessage.innerHTML = `<p style="color: white; font-size: 1.2rem;">Please select a brand.</p>`;
      resetProductDetails();
      return;
  }
  ...
  if (!productKey) {
      productDetails.style.display = 'flex';
      productMessage.innerHTML = `<p style="color: white; font-size: 1.2rem;">Please select a product.</p>`;
      resetProductDetails();
      return;
  }

  const product = watches[brand][productKey];
  productDetails.style.display = 'flex';
  ...
  document.getElementById('productDescription').textContent = product.caption;
}
